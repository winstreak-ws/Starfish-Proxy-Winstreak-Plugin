// Denicker System for Minecraft Proxy
// Detects nicked players by analyzing skin data
...
class DenickerSystem {
    ...
    registerHandlers() {
        ...
        setInterval(() => {
            this.updateTicks();
        }, 50); // run 20 times per second
    }
    
    handlePlayerInfo(data) {
        if (data.action !== 0) return; // only care about add player
        
        data.data.forEach(player => {
            if (!player.name || !player.UUID) return;
            
            // check if it's a version 1 UUID (NPCs)
            if (player.UUID.charAt(14) !== '1') return;
            
            // check if we've already parsed this player
            if (this.parsed.has(player.name)) return;
            
            // check if player has color in display name
            const displayName = player.displayName || player.name;
            const hasColor = displayName.includes('§');
            
            if (!hasColor) {
                // increment tick counter for players without color
                const currentTicks = this.noColorTicks.get(player.name) || 0;
                this.noColorTicks.set(player.name, currentTicks + 1);
                
                // wait 200 ticks (10 seconds) before processing players without color
                if (currentTicks < 200) return;
            }
            
            // parse properties for skin data
            ...
            // mark as parsed and remove from tick tracking
            this.parsed.add(player.name);
            this.noColorTicks.delete(player.name);
        });
    }
    
    checkPlayer(data) {
        // additional check for spawned entities
        const uuid = data.playerUUID;
        if (!uuid || uuid.charAt(14) !== '1') return;
        
        // we'll check this player when we get their player_info
    }
    ...
    parseSkinData(player, encodedData) {
        try {
            ...
            // extract skin hash from URL (4th element in split, like Java version)
            ...
            // check if using a known nick skin
            if (this.knownNickSkins.has(hash) && this.config.showFailedDenicks) {
    ...
    updateTicks() {
        // increment tick counters for players without color in their name
    ...
    sendAlert(message) {
        ...
        // send to current player (single player proxy)
        if (this.proxyAPI.currentPlayer) {
            this.proxyAPI.sendToClient(this.proxyAPI.currentPlayer.username, 'chat', {
    ...
    sendMessage(username, message) {
        this.proxyAPI.sendToClient(username, 'chat', {
...
module.exports = (proxyAPI) => {
    ...
    proxyAPI.on('clientPacket', (event) => {
        ...
        if (!message.startsWith('/denick')) return;
        ...
        switch (command) {
            case 'help':
                denicker.sendMessage(username, '§8[§cDenicker§8] Commands: /denick <help|status|toggle|failed>');
                break;
                
            case 'status':
                denicker.sendMessage(username, `§8[§cDenicker§8] Denicker is ${denicker.config.enabled ? '§aenabled' : '§cdisabled'}`);
                break;
                
            case 'toggle':
                denicker.config.enabled = !denicker.config.enabled;
                denicker.sendMessage(username, `§8[§cDenicker§8] Denicker ${denicker.config.enabled ? '§aenabled' : '§cdisabled'}`);
                break;
                
            case 'failed':
                denicker.config.showFailedDenicks = !denicker.config.showFailedDenicks;
                denicker.sendMessage(username, `§8[§cDenicker§8] Show failed denicks: ${denicker.config.showFailedDenicks ? '§aON' : '§cOFF'}`);
                break;
                
            case 'debug':
                denicker.config.debug = !denicker.config.debug;
                denicker.sendMessage(username, `§8[§cDenicker§8] Debug mode ${denicker.config.debug ? '§aenabled' : '§cdisabled'}`);
                break;
                
            default:
                denicker.sendMessage(username, '§8[§cDenicker§8] Unknown command. Use /denick help');
        }
