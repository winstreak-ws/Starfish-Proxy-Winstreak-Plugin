...
module.exports = (api) => {
    ...
    const configSchema = [
        ...
        {
            label: 'Audio Alerts',
            defaults: { audioAlerts: { enabled: false } },
            settings: [
                {
                    type: 'toggle',
...
class Denicker {
    ...
    registerHandlers() {
        this.api.on('playerRespawn', this.onPlayerRespawn.bind(this))
        this.api.on('playerJoin', this.onPlayerJoin.bind(this))
        this.api.on('playerLeave', this.onPlayerLeave.bind(this));
        this.api.on('playerDespawn', this.onPlayerDespawn.bind(this));
    }
    ...
    onPlayerDespawn({ name, uuid }) {
        if (!name || name === 'Unknown') return;
        this.nickData.delete(uuid);
        this.parsed.delete(name);
        this.noColorTicks.delete(name);
    }

    cleanName(name) {
        let newName = name;
        const regex = /§[0-9a-fk-or]$/;
    ...
    start() {
        if (!this.updateInterval) {
            this.updateInterval = setInterval(this.onUpdate.bind(this), 50);
        }
    }
    ...
    onUpdate() {
        ...
        for (const player of this.api.getPlayers()) {
            if (!player.name || typeof player.name !== 'string' || player.uuid.charAt(14) !== '1') continue;

            const cleanPlayerName = player.name.replace(/§./g, '');
            if (this.parsed.has(cleanPlayerName)) continue;

            const teamInfo = this.api.getPlayerTeam(cleanPlayerName);
            const prefix = teamInfo?.prefix || '';
            const suffix = teamInfo?.suffix || '';

            const hasColor = (prefix + suffix).includes('§');

            if (!hasColor) {
                const ticks = (this.noColorTicks.get(cleanPlayerName) || 0) + 1;
                this.noColorTicks.set(cleanPlayerName, ticks);
                if (ticks < 200) continue;
            }

            this.parseSkinData(player);
            this.parsed.add(cleanPlayerName);
            this.noColorTicks.delete(cleanPlayerName);
        }
    }
    ...
    parseSkinData(player) {
        ...
        try {
            ...
            const cleanPlayerName = name.replace(/§./g, '');
            const teamInfo = this.api.getPlayerTeam(cleanPlayerName);
    ...
    storeNickInfo(uuid, name, realName) {
        const cleanName = name.replace(/§./g, '');
        
        this.nickData.set(uuid, { 
            name: cleanName,
            realName 
        });
    ...
    tryUpdateNickTab(uuid) {
        ...
        const teamInfo = this.api.getPlayerTeam(nickInfo.name);
        ...
        setTimeout(() => {
            this.api.updatePlayerList(uuid, displayNameJson);
        }, 5);
    }

    sendAlert(playerName, realName) {
        setTimeout(() => {
            const cleanPlayerName = this.cleanName(playerName);
            const alertMsg = realName
                ? `§c${realName}§7 is nicked as §c${cleanPlayerName}§7.`
                : `§c${cleanPlayerName}§7 is nicked.`;
            this.api.sendChat(`${this.PLUGIN_PREFIX} ${alertMsg}`);

            if (this.config.audioAlerts.enabled) {
                this.api.playSound('note.pling');
            }
